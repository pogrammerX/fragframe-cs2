...
// How about some info on where to find these, ProDG?
...
public:
	...
	bool InheritsFrom(FNV1A_t uClassHash)
	{
		if (uClassHash == FNV1A::Hash(szName))
			return true;
		else if (pBaseClasses == nullptr)
			return false;

		for (int i = 0; i < nBaseClassesCount; i++)
		{
			auto& baseClass = pBaseClasses[i];
			if (baseClass.pClass->InheritsFrom(uClassHash))
				return true;
		}
...
public:
	...
	auto Element(int i) const { return m_pData[i]; }
